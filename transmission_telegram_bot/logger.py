...
def get_processors(timestamp_fmt: str | None = None) -> list[Processor]:
    return [
        structlog.stdlib.add_log_level,
        structlog.stdlib.add_logger_name,
        structlog.stdlib.PositionalArgumentsFormatter(),
        structlog.processors.TimeStamper(fmt=timestamp_fmt or "iso"),
...
def init_logger(
    ...
    log_level: str = "INFO",
    log_timestamp_format: str | None = None,
) -> None:
    level = getattr(logging, log_level.upper(), logging.INFO)
    renderer: ConsoleRenderer | JSONRenderer = json_renderer if log_format == "json" else console_renderer
